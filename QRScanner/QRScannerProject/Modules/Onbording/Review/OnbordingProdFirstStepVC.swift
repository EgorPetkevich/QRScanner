...
protocol OnbordingProdFirstStepViewModelProtocol {
    var delayAnimation: (() -> Void)? { get set }
    
    func continueButtonDidTap()
}

final class OnbordingProdFirstStepVC: UIViewController {
    
    private lazy var firstStepImageView: UIImageView =
    UIImageView(
        image: .Onbording.firstStepImage
    )
    
    private lazy var pageControl: UIPageControl =
        .onbordingPageControl(
            numOfPages: Onbording.numOfPages(),
            currentPage: Onbording.step(.first))
    
    private lazy var contentView: UIView = .contentView()
    private lazy var gradientView: UIView = .gradientView()
    private lazy var titleLabel: UILabel = .title("Welcome to<br>QR Code App")
    private lazy var subTitleLabel: UILabel = 
        .productionSubtitle("Simply open the app and start scanning QR codes<br>instantly&#8212no additional taps required.")
    ...
    private var viewModel: OnbordingProdFirstStepViewModelProtocol
    
    init(viewModel: OnbordingProdFirstStepViewModelProtocol) {
    ...
    private func setup() {
        ...
        view.addSubview(firstStepImageView)
        view.addSubview(contentView)
        
        firstStepImageView.addSubview(animationSubView)

...
final class BarcodeCell: UITableViewCell {
    
    private let fileManager = FileManagerService.instansce
    ...
    func setup(_ dto: BarcodeDTO) {
        typeImageView.image = getTypeImage(path: dto.id)
    ...
    private func setupConstraints() {
        ...
        typeImageView.cornerRadius = 7.0
    ...
    private func getTypeImage(path: String) -> UIImage {
        let image = fileManager.read(directory: .qrCodeImages, with: path)
        return image ?? .Result.stubImage
    }

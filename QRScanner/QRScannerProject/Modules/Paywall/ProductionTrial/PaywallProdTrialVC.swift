...
final class PaywallProdTrialVC: UIViewController {
    ...
    private lazy var subTitleLabel: UILabel = .paywallProdSubtitle("Start to continue QR code Scanner App with a 3 day\ntrial and \(viewModel.price ?? "7,99 $")/week")
    ...
    private func bind() {
        ...
        viewModel.getPrice = { [weak self] price in
            self?.subTitleLabel.text = "Start to continue QR code Scanner App with a 3 day\ntrial and \(price)/week"
        }
